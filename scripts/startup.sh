...
# Location of setup.conf to be used by set_option and all subsequent scripts. 
...
set_option() {
    if grep -Eq "^${1}.*" $CONFIG_FILE; then # check if option exists
        sed -i -e "/^${1}.*/d" # delete option if it exists
    fi
    echo "${1}=${2}" >>$CONFIG_FILE # add option (create the new option)
}
...
set_password() {
    ...
    if [[ "$PASSWORD1" == "$PASSWORD2" ]]; then
        ...
        echo -ne "ERROR! Passwords do not much. \n"
        set_password
    fi
}
...
root_check() {
    ...
    exit 0 # a succesfuly executed code should exit with code 0. Other values indicate an error
    fi
}

docker_check() { 
    # awk is used to search in files for a text containing a pattern and then the action is executed on the text that matches the pattern
...
# Gather username and password ti be used for installation.
userinfo () {
...
# Choose aur helper 
aurhelper () {
    # Let the user choose AUR helper from predifined list
...
# Starting functions every user will see
background_checks
clear
logo
userinfo
...
logo
desktopenv
# Set fixed options that installation uses if user choses the server installation
